...
Mail.prototype.send = function send(opts) {
    var mail = this;
...
Mail.prototype.render = function render(format, cb) {
    ...
    var view = views['mail/' + this.view + format];
    if (!view) {
        return cb(new Error('View ./app/views/mail/' + this.views + format +
        ' not found'));
    }
    if (!this.layout) {
    ...
    if (!layout) {
        return cb(new Error('View ./app/views/' + path + ' not found'));
    }
